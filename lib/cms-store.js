...
export async function saveMedia(pathname, body, contentType) {
  assertStorageConfigured();
  if (!pathname.startsWith(MEDIA_PREFIX)) throw new Error("Invalid media path");
  return put(pathname, body, {
    access: "private",
    addRandomSuffix: false,
...
export async function readMedia(pathname) {
  assertStorageConfigured();
  if (!pathname.startsWith(MEDIA_PREFIX) || pathname.includes("..") || pathname.includes("\\")) return null;
  return get(pathname, { access: "private", useCache: true });
}

export async function deleteMedia(pathname) {
  assertStorageConfigured();
  if (!pathname.startsWith(MEDIA_PREFIX) || pathname.includes("..") || pathname.includes("\\")) {
